...
- [x] core/pipeline.py：实现 run_project(config)，输出 summary
...
- [x] 基础测试：tests/test_pipeline.py + CI
...
- [x] 日志 & 进度 hook
- [x] CLI：`python -m glyphloom_core.cli translate --config config.yaml`（或 `glyphloom-core` 命令）
...
### 4.2 核心任务
**占位符识别**
- [x] 支持 `{name}`、`%s`、`{{VALUE}}`、`<tag>` 等模式
- [x] 可配置白名单
- [x] `extract_placeholders(text) -> Set[str]`
- [ ] 翻译流程中隐式还原占位符
...
## 10. 使用 Roadmap 的姿势
1. 每个 Stage 拆成 milestone，子任务拆 issue，issue 描述引用本文件编号。
2. 每完成一项勾选 Checklist，并在 commit 中提及。
3. 阶段末尾发布 tag + changelog，并视情况更新 `project_overview.md`。
4. 如果优先级变化，直接在本文档标注，提醒协作者同步。

核心原则：先跑通 Stage 1~2 的模板闭环与 QA；只要 pipeline 稳定，随时可以暂停迭代把现状当成可用工具，再逐步叠加引擎、GUI、专业功能。
